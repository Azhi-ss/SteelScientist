...
- **推理入口**：`scripts/run_steel_labelled_clusters_inference.sh` 调用 `regression/steel_labelled_clusters_inference.py`，默认读取 `/internfs/Zy/Steelllm/steel_labelled_clusters_inference_sample.csv`，加载 `/internfs/Zy/Steelllm/ckpt/steel_labelled_clusters_regression/*_best_model.pt`，输出 `regression/outputs/steel_labelled_clusters/inference_predictions.csv`。
