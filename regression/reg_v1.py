...
def plot_test_data(best_new_text_y, best_new_text_preds, text_result,
    ...
    plt.plot(np.arange(int(max(y_new_text)*0.1), int(max(y_new_text)*1.1)),
             np.arange(int(max(y_new_text)*0.1), int(max(y_new_text)*1.1)), '-', color='#A2555A')
    ...
    plt.plot(np.arange(int(max(y_exp)*0.1), int(max(y_exp)*1.1)),
             np.arange(int(max(y_exp)*0.1), int(max(y_exp)*1.1)), '-', color='#A2555A')
...
class CustomSimpleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        return {
            "labels": torch.tensor(self.targets[idx], dtype=torch.float32).to(device),
            "eles": torch.tensor(self.eles[idx], dtype=torch.float32).squeeze(0).to(device),
            "text_embeds": torch.tensor(self.text_embeds[idx], dtype=torch.float32).squeeze(0).to(device),
            "com_embeds": torch.tensor(self.com_embeds[idx], dtype=torch.float32).squeeze(0).to(device),
            "action_embeds": torch.tensor(self.action_embeds[idx], dtype=torch.float32).squeeze(0).to(device),
            "com_tsne_embeds": torch.tensor(self.com_tsne_embeds[idx], dtype=torch.float32).squeeze(0).to(device),
        }


class Unit(nn.Module):
    def __init__(self, normalization: str, in_features: int, out_features: int,
                 activation: str, dropout_prob: float):
        super().__init__()
...
class FcUnit(nn.Module):
    def __init__(self, num_features: int, normalization: Optional[str] = "null_norm",
                 activation: Optional[str] = 'relu', dropout_prob: Optional[float] = 0.1):
        super().__init__()
        if normalization == "layer_norm":
...
class Cnn1dUnit(nn.Module):
    def __init__(self, out_channels: int, kernel_size: Optional[int] = 3,
                 activation: Optional[str] = 'relu', stride: Optional[int] = 1,
                 padding: Optional[int] = 1, pooling: Optional[str] = 'max'):
        super().__init__()
        if pooling == "max":
        ...
        if activation == "leaky_relu":
            ...
            self.act_fn = ALL_ACT_LAYERS[activation]()
        self.conv1d = nn.LazyConv1d(out_channels=out_channels, kernel_size=kernel_size,
...
class Cnn2dUnit(nn.Module):
    def __init__(self, out_channels: int, kernel_size: Optional[int] = 2,
                 activation: Optional[str] = 'relu', stride: Optional[int] = 1,
                 padding: Optional[int] = 1, pooling: Optional[str] = 'max'):
        super().__init__()
        if pooling == "max":
        ...
        if activation == "leaky_relu":
            ...
            self.act_fn = ALL_ACT_LAYERS[activation]()
        self.conv2d = nn.LazyConv2d(out_channels=out_channels, kernel_size=kernel_size,
                                     stride=stride, padding=padding)
...
class CustomSimpleModel(nn.Module):
    def __init__(self, simple_layer_list, concat_layer_list, seq_embed_con1d_list,
                 seq_embed_fc_list, seq_embed_con2d_list, seq_embed_2d_fc_list,
                 simple_layer_drop_prob=0.0, concat_layer_drop_prob=0.0):
...
def train_function(config):
    ...
    val_dataloader = DataLoader(CustomSimpleDataset(**gen_data_class(test_data)), batch_size=len(test_data))
    ...
    with open('./outputs/reg_model.csv', 'a+') as csvfile:
        ...
        csvwriter.writerow([prop, train_result['r2'], val_result['r2'], best_new_text_result['r2'],
                           ...
                           conv_channels_layer_1, conv_channels_layer_2, embed_layer_num_32, embed_layer_num_16])

    session.report({
        "train_r2": train_result['r2'], "train_rmse": train_result['rmse'], "train_mae": train_result['mae'],
        "val_r2": val_result['r2'], "val_rmse": val_result['rmse'], "val_mae": val_result['mae'],
        "new_text_r2": best_new_text_result['r2'], "new_text_rmse": best_new_text_result['rmse'], "new_text_mae": best_new_text_result['mae'],
        "exp_r2": best_exp_result['r2'], "exp_rmse": best_exp_result['rmse'], "exp_mae": best_exp_result['mae']
    })
...
def tune_with_callback():
    tuner = tune.Tuner(
        ...
        tune_config=tune.TuneConfig(
            metric="val_r2", mode="max", num_samples=1, max_concurrent_trials=1,
        ),
        run_config=air.RunConfig(
            name="tune_reg",
            local_dir="./outputs/logs",
            callbacks=[WandbLoggerCallback(project="reg_model", mode="offline", tags=["1010"])]
        ),
        param_space={
